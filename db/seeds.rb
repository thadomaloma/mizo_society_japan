...
seed_password = ENV["SEED_SUPER_ADMIN_PASSWORD"].presence || ENV["SEED_USER_PASSWORD"].presence
password_required = super_admin.encrypted_password.blank? || reset_password
if Rails.env.production? && password_required && seed_password.blank?
  raise ArgumentError, "SEED_SUPER_ADMIN_PASSWORD is required when creating or resetting the production seed account"
end
...
super_admin.password = seed_password || "password123" if password_required
super_admin.save!

puts "Seeded one super admin: #{super_admin.email} (#{super_admin.role})"
puts "Complete the member profile at /profile/setup after the first sign-in." unless super_admin.profile_complete?
