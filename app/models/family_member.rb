class FamilyMember < ApplicationRecord
  ...
  def membership_fee_eligible?(on: Date.current)
    relationship.to_s.casecmp("Child").zero? && age_on(on).to_i >= 14
  end

  private

  def needs_membership_number?
    membership_number.blank? && relationship.to_s.casecmp("Child").zero? && member_profile&.membership_number.present?
  end

  def assign_membership_number
    prefix = "#{member_profile.membership_number}-C"
